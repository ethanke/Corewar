...
** Last update Tue Mar  8 19:17:14 2016 Guillaume SAGOT
*/
...
void		and_checkError(t_system *sys, int *values)
{
  if (values[2] != 1)
    ...
    printError(sys, "[ERROR] => \"AND\" instruction -> [:( BAD parameters)]\n");
}
...
{
  if (values[1] < 1)
    printError(sys, "[ERROR] => \"ZJMP\" instruction -> [:( BAD parameters)]\n");
  else if (values[1] != 0)
    printError(sys, "[ERROR] => \"ZJMP\" instruction -> DRAM necessary\n");
}

void 		ldi_checkError(t_system *sys, int *values)
{
  if (values[1] != 2 && values[2] != 3)
