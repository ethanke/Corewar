...
** Last update Sun Mar 13 12:37:46 2016 Gaëtan Léandre
*/
  ...
  if (arena->arena[process->pos + reg + 1] == T_REG)
    {
      ...
      arena->arena = put_param(arena->arena, pc_pos + 1 + (ind - 1)
  ...
  else
    {
      ...
      arena->arena = put_param(arena->arena,
			       circle(pc_pos + mod(ind, IDX_MOD), 0)
...
int		my_sti(t_arena *arena, t_process *process, int id, int pc_pos)
{
  ...
  nbr = take_what(arena->arena, process->pos, 1, pc_pos);
  reg = octet_to_read(arena->arena[process->pos]);
  ind = take_what(arena->arena, process->pos + reg + 1, 1, pc_pos);
  regB = octet_to_read(arena->arena[process->pos]);
  ind += take_what(arena->arena, process->pos + reg + regB + 2, 1, pc_pos);
